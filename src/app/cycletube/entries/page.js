...
export default function CycleTubeEntries() {
  ...
  async function fetchItems() {
    const res = await apiGet("/cycletube/dashboard/");
  ...
  async function fetchEntries() {
    ...
    if (filters.tube_item) query += `tube_item=${filters.tube_item}&`;
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((t) => {
      const name = `${t.size || ""} ${t.type || ""} ${t.brand || ""}`.toLowerCase();
      return name.includes(q);
    });
  }, [items, searchQuery]);

  const selectedItemObject = useMemo(() => {
    if (!filters.tube_item) return null;
    return items.find((t) => String(t.id) === String(filters.tube_item));
  }, [filters.tube_item, items]);
  ...
  return (
    <>
      ...
      <div className="fullscreen-container">
        <h1 className="page-title">🚲 Cycle Tube Entries</h1>
              ...
              <div className="custom-dropdown-container" ref={dropdownRef}>
                ...
                >
                  <span style={{ color: selectedItemObject ? '#0f172a' : '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {selectedItemObject 
                      ? `${selectedItemObject.size} ${selectedItemObject.type} ${selectedItemObject.brand}`
                      : 'Select Tube Item...'}
                  </span>
                ...
                {isDropdownOpen && (
                  <div className="custom-dropdown-menu">
                    ...
                    <div className="custom-dropdown-list">
                      <div 
                        className={`custom-dropdown-item ${!filters.tube_item ? 'selected' : ''}`}
                        onClick={() => { setFilters({...filters, tube_item: ''}); setIsDropdownOpen(false); }}
                      >
                        <span>All Items</span>
                      </div>
                      {filteredItems.map(t => (
                        <div 
                          key={t.id} 
                          className={`custom-dropdown-item ${String(filters.tube_item) === String(t.id) ? 'selected' : ''}`}
                          onClick={() => { setFilters({...filters, tube_item: t.id}); setIsDropdownOpen(false); }}
                        >
                          <div>
                            <span style={{ fontWeight: 700, color: '#1e293b' }}>{t.size}</span>
                            <span style={{ margin: '0 6px', color: '#64748b' }}>{t.type}</span>
                ...
                <tbody>
                  {entries.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        {item.tube_item_detail ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span style={{ fontWeight: 700, color: '#1d4ed8' }}>{item.tube_item_detail.size}</span>
                            <span style={{ color: '#475569', fontSize: '0.85rem' }}>{item.tube_item_detail.type}</span>
                            <span style={{ fontSize: '0.75rem', background: '#f1f5f9', color: '#64748b', padding: '2px 6px', borderRadius: '4px', fontWeight: 600 }}>{item.tube_item_detail.brand}</span>
                          </div>
                        ) : '-'}
                      </td>
                  ...
                  {!entries.length && (
                    <tr>
                      <td colSpan="8" style={{ textAlign: 'center', padding: '60px', color: '#94a3b8', fontWeight: 600, fontSize: '1.1rem' }}>
