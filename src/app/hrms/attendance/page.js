  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <div>
          {loading ? (
            <div className="card" style={{ textAlign: 'center', padding: '40px' }}>Loading attendance sheet...</div>
          ) : (
            (() => {
              const groups = {};
              ...
              if (sortedDepts.length === 0) {
                 return <div className="card" style={{ padding: '40px', textAlign: 'center' }}>No employees found.</div>;
              }

              return sortedDepts.map(dept => (
                <div key={dept} className="card" style={{ marginBottom: '30px' }}>
                  <div style={{ background: '#f8fafc', padding: '15px 20px', borderBottom: '2px solid #e2e8f0', borderRadius: '8px 8px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ margin: 0, color: '#1e293b', fontSize: '1.1rem' }}>
                      <i className="fas fa-users" style={{ marginRight: '8px', color: '#3b82f6' }}></i>
                      {dept} <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 'normal', marginLeft: '8px' }}>({groups[dept].length} workers)</span>
                    </h3>
                  </div>
                            ...
                            <tr key={emp.id} style={{ background: att.status === 'Absent' ? '#fff1f2' : att.status === 'Present' ? '#f0fdf4' : 'transparent' }}>
                              ...
                              <td>
                                {emp.employee_type === 'Company' ? (
                                  <span className="badge" style={{ background: '#8b5cf6', color: 'white', fontWeight: 800 }}>CO</span>
                                ) : (
                              ...
                              <td>
                                <select
                                  ...
                                  style={{
                                    ...
                                    borderColor: att.status === 'Present' ? '#bbf7d0' : att.status === 'Absent' ? '#fecaca' : '#fde68a'
                                  }}
                                  value={att.status}
                              ...
                              <td>
                                <input
                                  ...
                                  step="0.5"
                                  className="form-input"
                                  style={{ width: '70px', padding: '4px 8px', fontSize: '0.9rem', textAlign: 'center' }}
                              ...
                              <td>
                                <input
                                  ...
                                  step="0.5"
                                  className="form-input"
                                  style={{ width: '70px', padding: '4px 8px', fontSize: '0.9rem', textAlign: 'center', background: '#fffbeb', borderColor: '#fde68a' }}
